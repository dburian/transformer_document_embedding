...
We compile our training corpus same as we compile \Dataset{val-500k}.
...
\Dataset{train-1M}. We avoid introducing any new data, as the contextual
...
However, \Task{games} has much more documents to choose from.
...
unique papers in the dataset compared to \Task{aan}.
...
While \Task{wines} contains less documents and covers less topics, the
...
In the first round, Longformer's and SBERT's performance is underwhelming,
...
\Task{arxiv}, where we can imagine classifying the topic of a scholarly paper
...
times smaller embedding performs admirably well but worse than \Model{PV}. As
...
relatively close to the performance of \Model{PV}. This demonstrates, that in
...
\Task{wines}, \Task{games} has more documents in total but similar amount of
...
similarities of documents in \Task{wines} are sometimes based on few details
